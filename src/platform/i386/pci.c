...
pci_probe(void)
{
	...
	for(i=0; i<PCI_BUS_MAX; i++) {
		for(j=0; j<PCI_DEVICE_MAX; j++) {
			reg = pci_read(i, j, 0, 0x0);
			if (reg == 0xFFFFFFFF) continue;
			for(k=0; k<PCI_DATA_NUM; k++) devices[dev_num].data[k] = pci_read(i, j, 0, k<<2);
			devices[dev_num].bus       = (u32_t)i;
			devices[dev_num].dev       = (u32_t)j;
			devices[dev_num].func      = 0;
	...
	for(i=0; i<dev_num; i++) {
		printk("%x:%x vendor %x device %x class %x\n", devices[i].bus, devices[i].dev, devices[i].vendor, devices[i].device, devices[i].classcode);
		...
		if (devices[i].vendor == PCI_IVSHMEM_VENDOR && devices[i].device == PCI_IVSHMEM_DEVICE) {
			...
			printk("IVSHMEM DEVICE physical addr %x size %x\n", devices[i].bar[2].raw & 0xFFFFFFF0, (~(reg & 0xFFFFFFF0))+1);
		}
	}
}
